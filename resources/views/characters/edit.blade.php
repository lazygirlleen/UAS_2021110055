    ...
    <div class="col-12 px-5">
        ...
        <form action="{{ route('characters.update', $character->id) }}" method="POST">
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" id="name" placeholder="Name" name="name" value="{{ old('name', $character->name) }}" required>
            </div>
            ...
            <div class="form-group mt-3">
                ...
                <option disabled>Choose a type</option>
                    @foreach (['Bow', 'Sword', 'Claymore', 'Catalyst', 'Polearm'] as $type)
                        <option value="{{ $type }}" {{ old('weapon', $character->weapon) == $type ? 'selected' : '' }}>
                            {{ $type }}
                        </option>
                    @endforeach
                </select>
